...
import 'package:bourse_agricole/dependency_injection.dart'; // Pour 'sl'
...
class EcranConnexionClient extends StatefulWidget {
  ...
  @override
  State<EcranConnexionClient> createState() => _EcranConnexionClientState();
}

class _EcranConnexionClientState extends State<EcranConnexionClient> {
  final _emailController = TextEditingController(); 
  final _passController = TextEditingController();
  
  // Utilisation du Bloc via le Service Locator
  ...
  Widget build(BuildContext context) {
    return Scaffold(
      // On enveloppe avec BlocListener pour réagir aux changements d'état (Succès/Échec)
        ...
        listener: (context, state) {
          if (state is SuccesState<bool>) {
            Navigator.pushReplacement(
              context, 
              MaterialPageRoute(builder: (_) => const PageAccueilClient(role: "Acheteur"))
            );
            ...
            child: SingleChildScrollView(
              ...
              child: Column(
                children: [
                  ...
                  const Text("BAN", style: TextStyle(fontSize: 48, fontWeight: FontWeight.w900, color: Colors.white, letterSpacing: 4)),
                  const Text("Bourse Agricole Numérique", style: TextStyle(color: Colors.white70, fontWeight: FontWeight.bold, letterSpacing: 2)),
                  ...
                  // On utilise BlocBuilder pour changer le bouton en indicateur de chargement
                  BlocBuilder<AuthentifierBloc, BanState>(
                    ...
                    builder: (context, state) {
                      return _buildModelButton(
                        ...
                        onPressed: state is LoadingState ? null : _handleLogin,
                      );
                    },
                  ),

                  const SizedBox(height: 30),
                  const Text("Vous n'avez pas encore de compte ?", style: TextStyle(color: Colors.white, fontSize: 13)),
                  const SizedBox(height: 30),

                  _buildModelButton(
                    text: "CREEZ UN COMPTE",
                    isLoading: false,
                    onPressed: () => Navigator.push(context, MaterialPageRoute(builder: (_) => const CreationCompte())),
                  ),
  ...
  void _handleLogin() {
    ...
    // Déclenchement de l'événement via le Bloc
    _authBloc.add(AuthentifierEvent(
  ...
  Widget _buildPasswordInput() {
    return TextField(
      ...
      decoration: InputDecoration(
        ...
        prefixIcon: Icon(Icons.lock_outline, color: banPurpleBottom),
